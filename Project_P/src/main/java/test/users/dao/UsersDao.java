...
public class UsersDao {
	...
	public boolean delete(String id) {
		...
		try {
			...
			//실행할 sql문의 뼈대 구성
			String sql = "DELETE FROM p_users"
					+ " WHERE id=?";
	...
	public UsersDto getData(String id) {
	      ...
	      try {
	         conn = new DbcpBean().getConn();
	         String sql = "SELECT pwd, email, Pnum, regdate"
	               + " FROM p_users"
	               + " WHERE id=?";
	         pstmt = conn.prepareStatement(sql);
	         pstmt.setString(1, id);
	...
	public boolean insert(UsersDto dto) { //UsersDto에 id, pwd, email이 있다.
		...
		try {
			...
			conn = new DbcpBean().getConn();
			//실행할 sql문의 뼈대 구성
			String sql = "INSERT INTO p_users"
					+ " (id, pwd, email, pnum, regdate)"
					+ " VALUES(?, ?, ?, ?, sysdate)";
			//sql문의 ?에 바인딩 할 항목이 있다면 하기
			pstmt = conn.prepareStatement(sql);
