...
.container{
	display : grid;
	grid-template-rows : minmax(200px, auto) minmax(589px, auto) minmax(200px, auto);
...
<body>
		...
		<div class="signup_box">
			<form action="signup.jsp" method="post" id="signupForm">
				...
				<div>
					...
					<input class="form-control" type="text" name="pnum" id="pnum" placeholder="01012345678" />
					<div class="invalid-feedback">전화번호 형식에 맞게 입력하세요</div>
				</div>
				<br>
	...
	<script>
		...
		let isPnumValid = false;
		
		document.querySelector("#pnum").addEventListener("input", function(){
		    
		    this.classList.remove("is-valid");
		    this.classList.remove("is-invalid");

		    const inputPnum=this.value;
		   
		 
         	const reg=/^\d{3}\d{3,4}\d{4}$/;
			//만일 입력한 이메일이 정규표현식을 만족하지 않는다면
		    if(!reg.test(inputPnum)){
